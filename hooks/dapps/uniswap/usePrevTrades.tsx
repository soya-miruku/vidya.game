...
function UsePrevTrades(props: any) {
    ...
    if (!user) {
        return (
            <div>you gotta log in dummy</div>
        )
    }
    else
        return (
                ...
                <div>
                    {
                        tradeArray.map((id, key) => (
                            <VTable key={key}
                                borderWidth={0}
                                columns={[
                                    { label: 'TxHash', align: 'center', fontSize:'sm' },
                                    { label: 'From', align: 'center', fontSize:'sm' },
                                    { label: 'Amount', align: 'center', fontSize:'sm' },
                                    { label: 'To', align: 'center' , fontSize:'sm'},
                                    { label: 'Amount', align: 'center', fontSize:'sm' },
                                    { label: 'P&L', align: 'center', fontSize:'sm' }
                                ]}
                                data={[
                                    {
                                        0: <p className="text-accent-dark-100 "><a href={`https://etherscan.io/tx/${id.txHash}`} target="_blank">{id.txHash.slice(0, 5) + '...' + id.txHash.slice(-4, -1)}</a></p>,
                                        1: <p className="text-accent-dark-100 ">{id.token0}</p>,
                                        2: <p className="text-accent-dark-100 ">{id.amount0}</p>,
                                        3: <p className="text-accent-dark-100 ">{id.token1}</p>,
                                        4: <p className="text-accent-dark-100 ">{id.amount1}</p>,
