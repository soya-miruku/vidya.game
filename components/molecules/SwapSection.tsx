...
import { TokenSettingsModal } from './SwapSettingsModel';
...
export const SwapSection: React.FC<ISwapSectionProps> = ({defaultToken0="ETH", defaultToken1="VIDYA", showBorder, className}) => {
  ...
  const [ swapTokens, state ] = useSwap(token0?.address, token1?.address, slippage);
  ...
  return (
    <div className={classNames('flex flex-col  justify-center items-center', showBorder ? 'p-vsm border-[1px] rounded-lgr bg-light-100/[3%] border-light-300/30 backdrop-blur-sm': '', className)}>
      ...
      <AnimatePresenceModal>{showSettings && <TokenSettingsModal slippage={slippage} onSlippageChange={(e) => {
        setSlippage(() => e);
      }} onClose={() => setShowSettings(false)}/>}</AnimatePresenceModal>

      <VItemContainer widthSize='auto' heightSize='auto' dropShadow innerClassName='dark:!bg-dark-300/90'>
        <div className='flex flex-col sm:px-vmd pb-vmd pt-vsm px-vsm gap-y-vlrg'>
          <div className='flex justify-between items-center'>
          <VTitle type='h5'>Vidya Swap</VTitle>
