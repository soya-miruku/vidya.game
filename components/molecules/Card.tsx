    ...
    <div className={classNames('prose hover:-translate-y-6 transition-all duration-500 dark:hover:shadow-accent hover:shadow-accent rounded-lgr', url ? 'hover:cursor-pointer' : '')} onClick={() => url ? window.open(url, '_self') : null}>
      ...
      <VItemContainer center={center} showBorderTop={false} roundedTop={false} showBorder={bordered} widthSize={wide ? 'vxl' : 'vlg'} heightSize={ (sameType && !long) ? 'vhlf' : 'none'}>
        <div className={classNames('w-full flex flex-col justify-center gap-y-vmd px-vlrg pb-vlrg pt-vmd ', center ? 'items-center' : 'items-start', avatar ? 'mt-vmd pt-vmd' : 'mt-0','ml-0', footer ? 'gap-y-vsm' : 'gap-y-vsm', bordered ? '' : 'px-4 mt-0')}>
          <VTitle type='h4' className='min-h-[80px]'>{title}</VTitle>
          <VText className={classNames('max-h-[85px] overflow-y-auto text-body leading-7 min-h-[80px]', footer ? '' : 'min-h-[80px]')} size='md' weight='normal'>{subtitle}</VText>
          {footer && typeof(footer) === 'string' ? <VText className='!text-accent-dark-200' size='sm'>{footer}</VText> : footer}
        </div>
      </VItemContainer>
    </div>
  )
