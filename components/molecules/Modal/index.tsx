...
export const Modal = ({children, className, style, title, isOpen, onClose, width}: IModalProps) => {
  ...
  return (
    <Backdrop style={style} className={classNames(className, isMobileView ? 'h-screen' : 'h-full')}>
      <div className='flex justify-center items-center w-full h-screen' style={{
        height: isMobileView ? `${screen.availHeight}px` : '100%',
      }}>
        <div className={classNames('relative dark:bg-dark-200 bg-light-100 backdrop-blur-xl p-1 max-w-2xl rounded-lgr shadow-xl', width === 'sm' ? isMobileView ? 'w-full' :  'w-[27rem]' : isMobileView ? 'w-full' : 'w-11/12', isMobileView ? 'h-full' : 'h-auto')}>
