  ...
  return (
    <div className='w-full'>
      <div className='w-full h-full flex flex-wrap gap-x-4 py-12'>
        {(data || []).map((post, index) => {
          return (
            <BlogCard avatar={post.author.avatarUrl} key={index} image={post.meta?.featuredImage} title={post.meta.title} label={post.customValues?.label} subtitle={post.meta.description} footer={post.publishedAt} />
          )
        })}
      </div>
    </div>
