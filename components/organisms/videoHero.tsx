...
export const VideoHero = ({videoDesc, videoTitle, videoUrl, label, canEdit, centerTxt, showMouseIndicator, showGradientOverlay, autoplay=true, placeholderImage}: IVideoHeroProps) => {
  ...
  return (
    <div className="w-full h-full prose relative hover:cursor-pointer" onClick={() => !canEdit && setIsMuted(!isMuted)}>
      ...
      <div className={classNames('absolute w-1/2 h-[90%] flex justify-end px-vmd flex-col z-[1] gap-vmd', centerTxt ? 'items-center': 'items-start')}>
        {label && <VLabel className="border-gray-300/20 mb-vmd">{label}</VLabel>}
        {canEdit || typeof(videoTitle) !== 'string' ? <VRBTitle textAlign="left" overrideTextColor={isDarkMode ? true: false} type='h1' className="tracking-cta opacity-40" propName='videoTitle' ></VRBTitle> : <VTitle className="tracking-cta" overrideTextColor={isDarkMode ? true: false} type='title'>{videoTitle}</VTitle>}
        {canEdit || typeof(videoDesc !== 'string') ? <VRBText textAlign="left" overrideTextColor={isDarkMode ? true: false} size='lg' propName='videoDesc'></VRBText> :<VText className="w-full" overrideTextColor={isDarkMode ? true: false} size='lg'>{videoDesc}</VText>}
