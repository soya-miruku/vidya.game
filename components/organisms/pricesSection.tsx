...
export const PricesSection: React.FC<IPricesSectionProps> = ({tokenId = 'vidya', source="coinGecko"}) => {
  ...
  return (
    <div className='w-full h-full flex flex-col m-0 p-4 justify-center items-center flex-wrap'>
      <div className='w-full flex sm:gap-x-2 gap-x-1 gap-y-4 flex-wrap'>
        <PriceCard length={isMobileView ? 'sm' : 'md'} label='PRICE' price={data?.currentPrice[currencySelected]?.value?.toFixed(7) || 0} perctChange={data?.currentPrice[currencySelected].changePercentage24h || 0} increase="auto"></PriceCard>
