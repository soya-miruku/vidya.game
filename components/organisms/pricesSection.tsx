...
export const PricesSection: React.FC<IPricesSectionProps> = ({tokenId = 'vidya', source="coinGecko"}) => {
  ...
  return (
    <div className='w-full h-full flex flex-col justify-center items-center flex-wrap'>
      <div className='flex xl:gap-x-vxl lg:gap-x-vlrg md:gap-x-vmd gap-x-vsm gap-y-vlrg flex-wrap justify-center items-center p-vlrg'>
        <PriceCard length={(isMobileView || isTabletView) ? 'lg' : 'xs'} height={'xs'} label='PRICE' price={formatDecimals(data?.currentPrice[currencySelected]?.value)} perctChange={formatDecimals(data?.currentPrice[currencySelected].changePercentage24h)} increase="auto"></PriceCard>
        <PriceCard length={(isMobileView || isTabletView)  ? 'lg' : 'xs'} height={'xs'} label='MARKET CAP' price={formatPrice(data?.marketCap[currencySelected]?.value)} perctChange={data?.marketCap[currencySelected]?.changePercentage24h.toFixed(2) || 0} increase="auto"></PriceCard>
        <PriceCard length={(isMobileView || isTabletView)  ? 'lg' : 'xs'} height={'xs'} label='24HR VOL' price={formatPrice(data?.volume?.[currencySelected].value)} perctChange={data?.volume[currencySelected]?.changePercentage24h || 0} increase="auto"></PriceCard>
        <PriceCardWithCustomFooter length={(isMobileView || isTabletView)  ? 'lg' : 'xs'} label='TOTAL SUPPLY' price={formatPrice(data?.totalSupply)} footer={`${data?.circulatingSupply?.toFixed(2) || 0} circulating`}/>
      </div>
